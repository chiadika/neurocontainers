...
# Mirrors myrecon.OUTPUT_FRAME_ORDER_REVERSED_ATTRIBUTE. Duplicated rather
# than imported so this converter stays usable on its own, without the
# reconstruction module's runtime dependencies.
FRAME_ORDER_REVERSED_ATTRIBUTE = "myreconIceFrameOrderReversed"


def _frames_are_reversed(image):
    """Report whether the producer reversed the frame order against slice_dir.

    myrecon emits its volume with the frames reversed to compensate for
